...
export function DaySelector({
  ...
  const generateDays = (): DayOption[] => {
    ...
    // Fill with empty days to reach minimum of 7
    while (days.length < minDays) {
      days.push({
        day: "",
        date: 0,
        month: 0,
      ...
      <div className="flex gap-2 sm:justify-center overflow-x-auto pb-8">
        {days.map((dayOption, index) =>
          dayOption.isEmpty ? (
            <div
              key={index}
              className="relative flex flex-col items-center justify-center shrink-0 p-4 rounded-2xl w-20 h-24 bg-gray-200 border-dashed border-2 border-gray-300"
            />
          ) : (
            <button
              key={index}
